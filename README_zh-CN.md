...
Hive 是一个开源的多智能体协作平台。不同于单一 Agent 工具，Hive 赋予每个 AI Agent **持久身份**、**长期记忆**和**独立工作空间**——让它们组成一个团队协作工作，也和你一起工作。
...
### 🏢 数字员工，而非聊天机器人
Hive 的 Agent 是**组织的数字员工**。每个 Agent 了解完整的组织架构、可以发消息、委派任务、建立工作关系——就像一位新员工融入团队。
...
> **说明：** Hive 不在本地运行任何 AI 模型——所有 LLM 推理均由外部 API 提供商处理（OpenAI、Anthropic 等）。本地部署本质上是一个标准 Web 应用 + Docker 编排。
