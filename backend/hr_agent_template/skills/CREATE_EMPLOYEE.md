...
| `skill_names` | No | **NON-default platform skills only.** 14 defaults auto-install. Pass `[]` if only defaults. |
| `clawhub_slugs` | No | ClawHub skill slugs to install (e.g. `["market-research-agent"]`). Found via web_search on clawhub.ai. |
